...
def NLL(X, y, W, reg=0.0):
	...
	h = sigmoid(X @ W)
	temp = np.multiply(y, np.log(h)) + np.multiply((1. - y), np.log(1. - h))
...
def get_description(X, y, W):
	...
	y_label = predict(X, W)
	y_true = y
	accuracy = metrics.accuracy_score(y_true=y_true, y_pred=y_label)
	precision = metrics.precision_score(y_true=y_true, y_pred=y_label)
	recall = metrics.recall_score(y_true=y_true, y_pred=y_label)
	f1 = metrics.f1_score(y_true=y_true, y_pred=y_label)
	"*** END YOUR CODE HERE ***"
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	for i in range(len(reg_list)):
		W_opt, nll_list = grad_descent(X_train, y_train, reg=reg_list[i])
...
if __name__ == '__main__':
	...
	# NOTE: for logistic regression, ie only want images with label 0 or 1.
