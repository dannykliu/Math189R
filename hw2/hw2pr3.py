...
def linreg(X, y, reg=0.0):
	...
	eye[0, 0] = 0 # set weights of bias to zero
...
def find_RMSE(W, X, y):
	...
	y_pred = predict(W, X)
	MSE = np.linalg.norm(y-y_pred, 2) ** 2 / X.shape[0]
	return np.sqrt(MSE)
...
def norm_vs_lambda(X_train, y_train, X_val, y_val):
	...
	reg_list = np.random.uniform(0.0, 150.0, 150)
	reg_list.sort()
	W_list = [linreg(X_train, y_train, reg = lambda_) for lambda_ in reg_list]
...
if __name__ == '__main__':
	...
	# Read data
	df = pd.read_csv('https://math189su18.github.io/data/online_news_popularity.csv', sep=', ', engine='python')
